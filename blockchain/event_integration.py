...
async def emit_transaction_event(txid: str, transaction: Dict[str, Any], confirmed: bool = False, block_height: Optional[int] = None):
    ...
    try:
        ...
        # Include block height if available and transaction is confirmed
        if confirmed and block_height is not None:
            event_data['blockHeight'] = block_height
...
def emit_database_event(key: bytes, value: bytes, block_height: Optional[int] = None):
    """
    ...
    Args:
        ...
        block_height: Optional block height for confirmed transactions
    """
    # Decode key to determine type
    key_str = key.decode('utf-8')
    ...
    if key_str.startswith('tx:'):
        ...
        try:
            tx_data = json.loads(value.decode())
            # Use asyncio to run the async emit function
            asyncio.create_task(emit_transaction_event(txid, tx_data, confirmed=True, block_height=block_height))
        except Exception as e:
            logger.error(f"Error emitting event for transaction {txid}: {e}")
